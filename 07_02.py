...
class Directory:
    ...
    def find_answer(self, needed, answers):
        if self.get_total_size() > needed:
            answers.append(self.get_total_size())
        for i in self.directories:
            i.find_answer(needed, answers)
...
def main():
    ...
    empty = 70000000 - root.get_total_size()
    needed = 30000000 - empty
    answers = []
    root.find_answer(needed, answers)
    answers.sort()
    print(empty, needed, answers)
    print(min(answers))
